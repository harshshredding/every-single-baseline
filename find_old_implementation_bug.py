...
for sample_new, sample_old in zip(valid_samples_new, valid_samples_old):
    ...
    if collect_new[1] != collect_old[1]:
        batch_encoding : BatchEncoding = collect_new[0]
        char_spans = []
        for token_idx in range(len(batch_encoding.word_ids())):
            char_spans.append(batch_encoding.token_to_chars(batch_or_token_index=0, token_index=token_idx))
        print()
        print()
        print(char_spans)
        print(
        ...
        raise RuntimeError("found roberta bio error")
